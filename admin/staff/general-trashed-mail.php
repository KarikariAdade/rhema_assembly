            ...
            <div class="box-body no-padding">
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  <tbody>
                    <?php
                    $inbox_sql = "SELECT * FROM mailbox WHERE trash ='Yes' ORDER BY id DESC";
                    ...
                    if (mysqli_num_rows($inbox_query) > 0) {
                      while ($row = mysqli_fetch_assoc($inbox_query)) {
                       $contact_id = $row['id'];
                        ...
                        if (strlen($contact_message) > 60){
        $contact_message = wordwrap($contact_message,60);
        ...
        $contact_message = $contact_message[0]."...";
                  ...
                  <tr>
                    <td style="display: inline-flex;">
                      <form method="POST" action="includes/general-restore-mail.php">
                        <input type="hidden" name="restore_id" value="<?php echo $contact_id; ?>">
                      <button type="submit" name="general-mail-restore-btn" onclick="return confirm('Are you sure you want to restore this message?')" style="background-color: transparent;border:none;" class="btn btn-xs btn-danger"><span style="color:#dd4b39;" class="fa fa-recycle"></span></button>
                    </form>
                    <form method="POST" action="includes/general-delete-mail.php">
                        <input type="hidden" name="delete_id" value="<?php echo $contact_id; ?>">
                      <button type="submit" name="general-mail-delete-btn" onclick="return confirm('Are you sure you want to delete this message? Deleted messages cannot be restored.')" style="background-color: transparent;border:none;" class="btn btn-xs btn-danger"><span style="color:#dd4b39;" class="fa fa-trash"></span></button>
                    </form>
                  </td>
