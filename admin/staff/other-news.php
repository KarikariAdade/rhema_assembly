  ...
  <body class="hold-transition skin-blue sidebar-mini">
  	<div class="wrapper">
          ...
          $start_from = ($page - 1)*$record_per_page;
                $sql = "SELECT * FROM news WHERE news_category ='Other News' ORDER BY id DESC LIMIT $start_from, $record_per_page";
                $query = mysqli_query($conn,$sql);
                if (mysqli_num_rows($query) > 0) {
                  while ($row = mysqli_fetch_assoc($query)) {
                    $id = $row['id'];
      ...
      <?php
                  ...
                  echo "<h2>There are no posts under <strong>'Other News'</strong> category yet.</h2>";
                }
      ?>
      ...
      <?php
      $page_query = "SELECT * FROM news WHERE news_category ='Other News' ORDER BY id DESC";
                $page_query_sql = mysqli_query($conn, $page_query);
                $total_records = mysqli_num_rows($page_query_sql);
                $total_pages = ceil($total_records / $record_per_page);

                for ($i=1; $i<=$total_pages; $i++) { 
                  echo "<a class='pagination' href='other-news.php?page=".$i."'>".$i."</a>";
                }
                ?>
              <?php endif;?>
  							</p></div>
                ...
                <div class="box-body no-padding">
                  <ul class="nav nav-pills nav-stacked">
                    <li><a href="general-news.php"><i class="fa fa-church"></i> General News
                      ...
                      <li class="active"><a href="other-news.php"><i class="fa fa-male"></i> Other News <span class="label label-primary pull-right"><?php echo $other_news_counter; ?></span></a></li>
                    </ul>
                  </div>
                  <!-- /.box-body -->
                </div>
