  ...
  <div class="row">
     <div class="col-md-8" style="padding-right: 5%;">
      <div class="current_series">
        <?php
         $current_sql = "SELECT * FROM sermon WHERE service_type = 'Women Ministry' ORDER BY id DESC LIMIT 1";
    $current_query = mysqli_query($conn, $current_sql);
        ?>
        <?php if (mysqli_fetch_assoc($current_query) <= 0): ?>
          <h2>There are no sermons under Women's Ministry yet.....<br><a href="add-sermon.php"><?=(($position == "Presiding Elder" || $position == "Elder" || $position == "Secretary"))?'Add One':''?></a></h2>
      <?php else:?>
    ...
    $current_sql = "SELECT * FROM sermon WHERE service_type='Women Ministry' ORDER BY id DESC LIMIT $start_from, $record_per_page";
    $current_query = mysqli_query($conn, $current_sql);
    if (mysqli_num_rows($current_query) > 0) {
      while ($row = mysqli_fetch_assoc($current_query)) {
        $title = $row['title'];
    ...
    <div style="margin-top: 5%;">
      <p align="center">
         <?php
      $page_query = "SELECT * FROM sermon WHERE service_type='Women Ministry' ORDER BY id DESC";
                $page_query_sql = mysqli_query($conn, $page_query);
                $total_records = mysqli_num_rows($page_query_sql);
                $total_pages = ceil($total_records / $record_per_page);

                for ($i=1; $i<=$total_pages; $i++) { 
                  echo "<a class='pagination' href='women-sermon.php?page=".$i."'>".$i."</a>";
                }
                ?>
      </p>
    </div>
                ...
                <div class="box-body no-padding">
                  <ul class="nav nav-pills nav-stacked">
                    <li><a href="sunday-sermon.php"><i class="fa fa-church"></i> Sunday Service
                      ...
                      <li class="active"><a href="women-sermon.php"><i class="fa fa-female"></i> Women's Ministry <span class="label label-primary pull-right"><?php echo $women_counter; ?></span></a>
                      </li>
