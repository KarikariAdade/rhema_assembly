...
if (mysqli_num_rows($query) > 0) {
  ...
  $to = date('Y-m-31 23:59:59', strtotime("$year-$month"));
                ...
                $sql = "SELECT * FROM announcement WHERE date >= '$from' AND date <= '$to' ORDER BY id DESC LIMIT $start_from, $record_per_page";
                $query = mysqli_query($conn,$sql);
                if (mysqli_num_rows($query) > 0) {
                  while ($row = mysqli_fetch_assoc($query)) {
                    $id = $row['id'];
      ...
      <?php
      $page_query = "SELECT * FROM announcement WHERE date >= '$from' AND date <= '$to' ORDER BY id DESC";
                $page_query_sql = mysqli_query($conn, $page_query);
                $total_records = mysqli_num_rows($page_query_sql);
                $total_pages = ceil($total_records / $record_per_page);

                for ($i=1; $i<=$total_pages; $i++) { 
                  echo "<a class='pagination' href='announcement-archive.php?month=$month&year=$year&page=".$i."'>".$i."</a>";
                }
                ?>
              <?php endif;?>
            </p>
