  ...
  <body class="hold-transition skin-blue sidebar-mini">
  	<div class="wrapper">
          ...
          //set from and to dates
               ...
               $sql = "SELECT * FROM news WHERE news_date >= '$from' AND news_date <= '$to' ORDER BY id DESC";
    ...
    //collect month and year data
      ...
      // echo $from;
              ...
              $sql = "SELECT * FROM news WHERE news_date >= '$from' AND news_date <= '$to' ORDER BY id DESC LIMIT $start_from, $record_per_page";
              $query = mysqli_query($conn,$sql);
              ...
              <div style="margin-top: 5%;">
                <p align="center">
                  <?php if(mysqli_num_rows($query) > 1): ?>
                   <?php
                   $page_query = "SELECT * FROM news WHERE news_date >= '$from' AND news_date <= '$to' ORDER BY id DESC";
                   $page_query_sql = mysqli_query($conn, $page_query);
                   $total_records = mysqli_num_rows($page_query_sql);
                   $total_pages = ceil($total_records / $record_per_page);

                   for ($i=1; $i<=$total_pages; $i++) { 
                    echo "<a class='pagination' href='news-archive.php?page=".$i."&month=$month&year=$year'>".$i."</a>";
                  }
                  ?>
                <?php endif;?>
              </p>
