   ...
   <section class="content-header">
	...
	<div class="row">
        ...
        <?php
         ...
         $current_sql = "SELECT * FROM sermon ORDER BY id DESC LIMIT 1";
    $current_query = mysqli_query($conn, $current_sql);
        ?>
         <?php if (mysqli_fetch_assoc($current_query) <= 0): ?>
          <h2>There are no sermons yet.....<br> <a href="add-sermon.php"><?=(($position == "Presiding Elder" || $position == "Elder" || $position == "Secretary"))?'Add One':''?></a></h2>
      <?php else:?>
        ...
        $sermon_notes = $row['sermon_notes'];
           ...
           if (strlen($sermon_notes) > 400){
        $sermon_notes = wordwrap($sermon_notes,400);
      ...
      $record_per_page = 15;
		...
		$current_sql = "SELECT * FROM sermon ORDER BY id DESC LIMIT $start_from, $record_per_page";
		$current_query = mysqli_query($conn, $current_sql);
		if (mysqli_num_rows($current_query) > 0) {
			while ($row = mysqli_fetch_assoc($current_query)) {
        $sermon_id = $row['id'];
    ...
    <div style="margin-top: 5%;">
      <p align="center">
         <?php
      $page_query = "SELECT * FROM sermon ORDER BY id DESC";
                $page_query_sql = mysqli_query($conn, $page_query);
                $total_records = mysqli_num_rows($page_query_sql);
                $total_pages = ceil($total_records / $record_per_page);

                for ($i=1; $i<=$total_pages; $i++) { 
                  echo "<a class='pagination' href='view-sermons.php?page=".$i."'>".$i."</a>";
                }
                ?>
      </p>
    </div>
