  ...
  <body class="hold-transition skin-blue sidebar-mini">
  	<div class="wrapper">
          ...
          $start_from = ($page - 1) * $record_per_page;
  							$sql = "SELECT * FROM announcement WHERE category = 'Women Ministry' ORDER BY id DESC LIMIT $start_from, $record_per_page";
  							$query = mysqli_query($conn, $sql);
  							if (mysqli_num_rows($query) > 0) {
  								while ($row = mysqli_fetch_assoc($query)) {
  									$id = $row['id'];
                  ...
                  echo "<h3>There are no announcements under <strong>Women's Ministry</strong> yet.</h3>";
                }
  							?>
      ...
      <?php
      $page_query = "SELECT * FROM announcement WHERE category = 'Women Ministry' ORDER BY id DESC";
                $page_query_sql = mysqli_query($conn, $page_query);
                $total_records = mysqli_num_rows($page_query_sql);
                $total_pages = ceil($total_records / $record_per_page);

                for ($i=1; $i<=$total_pages; $i++) { 
                  echo "<a class='pagination' href='women-announcement.php?page=".$i."'>".$i."</a>";
                }
                ?>
              <?php endif;?>
            </p>
                  ...
                  <ul class="nav nav-pills nav-stacked">
                    <li><a href="general-announcement.php"><i class="fa fa-church"></i> General Announcements
                      ...
                      <li class="active"><a href="women-announcement.php"><i class="fa fa-female"></i> Women's Ministry <span class="label label-primary pull-right"><?php echo $women_counter; ?></span></a>
                      </li>
